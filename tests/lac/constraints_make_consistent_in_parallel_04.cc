...
test()
{
  ...
  const auto show_constraints_1_2 = [&]() {
    for (unsigned int p = 0;
         p < Utilities::MPI::n_mpi_processes(MPI_COMM_WORLD);
         ++p)
      {
        if (p == Utilities::MPI::this_mpi_process(MPI_COMM_WORLD))
          {
            deallog << "On process " << p << std::endl;

            deallog << "What process " << p << " believes about DoF 1:\n";
            for (const auto &c : constraints.get_lines())
              if (c.index == 1)
                for (const auto &entry : c.entries)
                  deallog << "    constrained against " << entry.first
                          << " with weight " << entry.second << '\n';
            deallog << "What process " << p << " believes about DoF 2:\n";
            for (const auto &c : constraints.get_lines())
              if (c.index == 2)
                for (const auto &entry : c.entries)
                  deallog << "    constrained against " << entry.first
