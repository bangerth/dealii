  ...
  public:
                         ...
                         const unsigned int /*component*/ = 0) const override
    {
      ...
      const Point<2> p_long_lat(
        ...
        std::asin(p[2] / std::sqrt(p[0] * p[0] + p[1] * p[1])) * 360 /
          (2 * numbers::PI));

      // REMOVE
      std::cout << p_long_lat << std::endl;
      Assert(numbers::is_finite(p_long_lat[0]), ExcInternalError());
      Assert(numbers::is_finite(p_long_lat[1]), ExcInternalError());
